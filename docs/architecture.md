...
`pro-lowcode-platform-front/LlmConfig` 当前采用直接静态 JSON 消费：由 `VUE_APP_CDN_PATH + VUE_APP_LLM_CATALOG_MANIFEST_PATH`（或部署时的 `window.__APP_ENV__`）定位 manifest，前端用 Web Crypto 校验搜索索引和按需 provider 分片，再用自身组件库渲染与投影表单。独立 `index.html` 继续用于完整浏览，不参与创建页通信；目录保留通用 iframe picker，但该前端不使用。`baiteda-app` 不访问目录 CDN，只保存 tenant deployment、目录绑定和 FDE 所需的白名单能力投影；已有模型运行不依赖浏览器或 CDN 实时可用。具体删除/保留边界见[创建模型联动设计](llm-config-integration.md)。
