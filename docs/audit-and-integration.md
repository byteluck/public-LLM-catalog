...
2026-08-03 再次读取了要求中的全部文件和 `LlmConfig/` 目录。审计以读取时 HEAD 为准；`baiteda-app` 与 FDE 的必读文件均未带工作区修改，本目录不修改这两个仓库：
...
`LlmConfig` 的创建模型联动已进一步细化为[目录绑定与租户 deployment 设计](llm-config-integration.md)。选择入口直接读取可配置 CDN 的 manifest、搜索索引和按需 provider 分片，用前端自身组件渲染，不再调用后端公开目录搜索，也不再使用 iframe/postMessage；选择后当前仍只做安全预填，没有持久化 catalog identity 和版本。完整目标是在后续保存契约中接收经过白名单校验的绑定与可选能力快照，后端始终不访问目录 CDN。
...
| `.../vo/ai/LlmModelPublicCatalogTemplateVo.java`、`PageVo.java`、对应 Query BO | 删除；浏览、搜索和详情由前端直接读取 CDN 静态 JSON 完成，不再提供后端模板 API |
...
| 文件 | 所需变更 |
| --- | --- |
| `.env`、`index.html`、`public/index.html` | 增加 `VUE_APP_LLM_CATALOG_MANIFEST_PATH=/LLM_catalog/manifest.json`，并保留完整目录页面路径；与 `VUE_APP_CDN_PATH` 一起注入 `window.__APP_ENV__` |
| `src/views/LlmConfig/publicCatalogClient.ts`（新增） | 每次先取 manifest；按哈希缓存 search index，按需读取 provider 分片；校验 size/SHA-256，失败回退最后成功快照，请求不带凭据 |
| `src/views/LlmConfig/publicCatalogBridge.ts` | 对已校验的 provider 分片做公开字段白名单投影和表单映射；unknown 不转 false，租户字段不进入投影 |
...
| contract/unit test | 增加私有 CDN 地址、manifest 增量缓存、哈希失败、上次成功回退、unknown 保留、private override、capability gating、sampling 分层及无 URL/Key 进入公开投影的测试 |
